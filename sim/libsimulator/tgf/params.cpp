...
struct tGfParmHandle
{
	...
	GF_TAILQ_ENTRY (struct tGfParmHandle)	linkHandle;	/**< Next configuration handle */
};


GF_TAILQ_HEAD (parmHead, struct tGfParmHandle);
...
static void parmReleaseHandle (struct tGfParmHandle *parmHandle);
...
GfParmShutdown (void)
{
    struct tGfParmHandle	*parmHandle;
...
{
	struct tGfParmHandle *parmHandle = (struct tGfParmHandle *)userData;
			     ...
			     const XML_Char * /* publicId */)
{
    ...
    parmHandle = (struct tGfParmHandle *)XML_GetUserData (mainparser);
    conf = parmHandle->conf;
...
static int
parseXml (struct tGfParmHandle *parmHandle, char *buf, int len, int done)
{
...
static int
parserXmlInit (struct tGfParmHandle *parmHandle)
{
...
{
    struct parmHeader	*conf;
    struct tGfParmHandle	*parmHandle = NULL;
    ...
    /* Handle creation */
    parmHandle = (struct tGfParmHandle *) calloc (1, sizeof (struct tGfParmHandle));
    if (!parmHandle) {
	GfError ("gfParmReadBuf: calloc (1, %d) failed\n", sizeof (struct tGfParmHandle));
	goto bailout;
    }
 ...
 */
static int
xmlGetOuputLine (struct tGfParmHandle *parmHandle, char *buffer, int /* size */)
{
...
*/
int
GfParmWriteBuf (GfParmHandle *handle, char *buf, int size)
{
    struct tGfParmHandle	*parmHandle = (struct tGfParmHandle *)handle;
    struct parmHeader	*conf;
    char		line[LINE_SZ];
    int			len;
    int			curSize;
...
*/
void
GfParmSetDTD (GfParmHandle *parmHandle, char *dtd, char*header)
{
    struct tGfParmHandle	*handle = (struct tGfParmHandle *)parmHandle;
...
*/
void
GfParmRemove (GfParmHandle *parmHandle, char *sectionName, char *paramName)
{
    struct tGfParmHandle	*handle = (struct tGfParmHandle *)parmHandle;
    struct parmHeader	*conf;
...
*/
void
GfParmClean (GfParmHandle *parmHandle)
{
    struct tGfParmHandle	*handle = (struct tGfParmHandle *)parmHandle;
    struct parmHeader	*conf;
...
/** Clean the parms and release the handle without updating the file
    ...
    @return	none
*/
void GfParmReleaseHandle (GfParmHandle *parmHandle)
{
	struct tGfParmHandle *handle = (struct tGfParmHandle *)parmHandle;
...
*/
char *
GfParmGetName (GfParmHandle *handle)
{
    struct tGfParmHandle	*parmHandle = (struct tGfParmHandle *)handle;
    struct parmHeader	*conf = parmHandle->conf;
...
*/
char *
GfParmGetFileName (GfParmHandle *handle)
{
    struct tGfParmHandle	*parmHandle = (struct tGfParmHandle *)handle;
    struct parmHeader	*conf = parmHandle->conf;
 ...
 */
int
GfParmGetEltNb (GfParmHandle *handle, char *path)
{
    struct tGfParmHandle	*parmHandle = (struct tGfParmHandle *)handle;
    struct parmHeader	*conf = parmHandle->conf;
    struct section	*section;
 ...
 */
int
GfParmListSeekFirst (GfParmHandle *handle, char *path)
{
 ...
 */
int
GfParmListSeekNext (GfParmHandle *handle, char *path)
{
    struct tGfParmHandle	*parmHandle = (struct tGfParmHandle *)handle;
    struct parmHeader	*conf = parmHandle->conf;
    struct section	*section;
 ...
 */
int
GfParmListClean (GfParmHandle *handle, char *path)
{
    struct tGfParmHandle	*parmHandle = (struct tGfParmHandle *)handle;
    struct parmHeader	*conf = parmHandle->conf;
 ...
 */
char *
GfParmListGetCurEltName (GfParmHandle *handle, char *path)
{
	struct tGfParmHandle *parmHandle = (struct tGfParmHandle *)handle;
	struct parmHeader *conf = parmHandle->conf;
	struct section *section;
...
*/
char *
GfParmGetStr (GfParmHandle *parmHandle, char *path, char *key, char *deflt)
{
	struct param *param;
	struct tGfParmHandle *handle = (struct tGfParmHandle *)parmHandle;
	struct parmHeader *conf;
...
*/
char *
GfParmGetCurStr (GfParmHandle *handle, char *path, char *key, char *deflt)
{
    struct tGfParmHandle	*parmHandle = (struct tGfParmHandle *)handle;
    struct parmHeader	*conf = parmHandle->conf;
    struct section	*section;
 ...
 */
tdble
GfParmGetNum (GfParmHandle *handle, char *path, char *key, char *unit, tdble deflt)
{
    struct tGfParmHandle	*parmHandle = (struct tGfParmHandle *)handle;
    struct parmHeader	*conf = parmHandle->conf;
    struct param	*param;
 ...
 */
tdble
GfParmGetCurNum (GfParmHandle *handle, char *path, char *key, char *unit, tdble deflt)
{
    struct tGfParmHandle	*parmHandle = (struct tGfParmHandle *)handle;
    struct parmHeader	*conf = parmHandle->conf;
    struct section	*section;
 ...
 */
int
GfParmSetStr(GfParmHandle *handle, char *path, char *key, char *val)
{
    struct tGfParmHandle	*parmHandle = (struct tGfParmHandle *)handle;
    struct parmHeader	*conf = parmHandle->conf;
    struct param	*param;
 ...
 */
int
GfParmSetCurStr(GfParmHandle *handle, char *path, char *key, char *val)
{
    struct tGfParmHandle	*parmHandle = (struct tGfParmHandle *)handle;
    struct parmHeader	*conf = parmHandle->conf;
    struct section	*section;
 ...
 */
int
GfParmSetNum(GfParmHandle *handle, char *path, char *key, char *unit, tdble val)
{
    struct tGfParmHandle	*parmHandle = (struct tGfParmHandle *)handle;
    struct parmHeader	*conf = parmHandle->conf;
    struct param	*param;
 ...
 */
int
GfParmSetNumEx(GfParmHandle *handle, char *path, char *key, char *unit, tdble val, tdble min, tdble max)
{
    struct tGfParmHandle	*parmHandle = (struct tGfParmHandle *)handle;
    struct parmHeader	*conf = parmHandle->conf;
    struct param	*param;
 ...
 */
int
GfParmSetCurNum(GfParmHandle *handle, char *path, char *key, char *unit, tdble val)
{
    struct tGfParmHandle	*parmHandle = (struct tGfParmHandle *)handle;
    struct parmHeader	*conf = parmHandle->conf;
    struct section	*section;
 ...
 */
int
GfParmCheckHandle(GfParmHandle *ref, GfParmHandle *tgt)
{
    struct tGfParmHandle	*parmHandleRef = (struct tGfParmHandle *)ref;
    struct tGfParmHandle	*parmHandle = (struct tGfParmHandle *)tgt;
...
static void
insertParamMerge (struct tGfParmHandle *parmHandle, char *path, struct param *paramRef, struct param *param)
{
    struct parmHeader	*conf = parmHandle->conf;
...
static void
insertParam (struct tGfParmHandle *parmHandle, char *path, struct param *param)
{
    struct parmHeader	*conf = parmHandle->conf;
    struct param	*paramNew;
    struct within	*within;
 ...
 */
GfParmHandle *
GfParmMergeHandles(GfParmHandle *ref, GfParmHandle *tgt, int mode)
{
    struct tGfParmHandle	*parmHandleRef = (struct tGfParmHandle *)ref;
    struct tGfParmHandle	*parmHandleTgt = (struct tGfParmHandle *)tgt;
    struct tGfParmHandle	*parmHandleOut;
    struct parmHeader	*confRef = parmHandleRef->conf;
    ...
    /* Handle creation */
    parmHandleOut = (struct tGfParmHandle *) calloc (1, sizeof (struct tGfParmHandle));
    if (!parmHandleOut) {
	GfError ("gfParmReadBuf: calloc (1, %d) failed\n", sizeof (struct tGfParmHandle));
    ...
    return (GfParmHandle*)parmHandleOut;
}
 ...
 */
int
GfParmGetNumBoundaries(GfParmHandle *handle, char *path, char *key, tdble *min, tdble *max)
{
    struct tGfParmHandle	*parmHandle = (struct tGfParmHandle *)handle;
    struct parmHeader	*conf = parmHandle->conf;
    struct param	*param;
