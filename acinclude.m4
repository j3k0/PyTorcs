...
AC_DEFUN([OPENRACING_USE_STLPORT],
[AC_ARG_WITH(stlport, 
...
AC_DEFUN([OPENRACING_CHECK_OGRE], [
    PKG_CHECK_MODULES(OGRE, OGRE >= 1.4.0,,exit)
    AC_SUBST(OGRE_CFLAGS)
    AC_SUBST(OGRE_LIBS)
])

AC_DEFUN([OPENRACING_CHECK_OIS], [
    PKG_CHECK_MODULES(OIS, OIS >= 1.0.0,,exit)
    AC_SUBST(OIS_CFLAGS)
    AC_SUBST(OIS_LIBS)
])

AC_DEFUN([OPENRACING_CHECK_MYGUI], [
    PKG_CHECK_MODULES(MYGUI, MyGUI >= 2.1.0,,exit)
    AC_SUBST(MYGUI_CFLAGS)
    AC_SUBST(MYGUI_LIBS)
])
